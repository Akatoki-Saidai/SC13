...
def calculate_heading(x, y, z):
    """
    Calculate heading from magnetic sensor values.
    Args:
        x (float): X-axis magnetic sensor value.
        y (float): Y-axis magnetic sensor value.
        z (float): Z-axis magnetic sensor value.
    Returns:
    ...
    if y > 0:
        ...
        direction = (math.atan2(x, y) + 2 * math.pi) * 180 / math.pi
    else:
        if x < 0:
            direction = 270.0
        else:
