...
t = 0
while True:
    for i in range(600):#60秒実行するようにしている
        ...
        t0 = +2.0 * (w * x + y * z)
        t1 = +1.0 - 2.0 * (x * x + ysqr)
        roll = np.atan2(t0, t1)*180/math.pi
        ...
        pitch = np.arcsin(t2)*180/math.pi

        # yaw (z-axis rotation)
        t3 = +2.0 * (w * z + x * y)
        t4 = +1.0 - 2.0 * (ysqr + z * z);  
        yaw = np.arctan2(t3, t4) *180/math.pi

        print("roll :%f", roll)
        print("pitch:%f", pitch)
        print("yaw  :%f", yaw)
        file.write(str(t) + ',' + str(yaw) + ',' + str(roll) + ',' + str(pitch) + '\n')
        time.sleep(0.1)
        t +=0.1
        i += 1
    break
